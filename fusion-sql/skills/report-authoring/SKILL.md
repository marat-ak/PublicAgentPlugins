...
list, and aggregation — get them wrong and you rebuild the model (the customer-A requirements-doc failure). Ask the
...
- **BANNED in templates:** cross-group XPath aggregation in cells (`sum(/DS/G[…=current()…]/QTY)`
  ...
  even when it renders (the customer-A pivot) — pivot in SQL, keep the template a flat `<?FIELD?>` table.
...
**Iterate LOCALLY until the user approves (RTF).** When modifying a layout AFTER a rendered result was
already shared: edit → render **locally** (`renderTemplate`, reuse the real data XML `dataFileId` from
the earlier pod run) → Read + verify → share the new PDF → repeat until the user **approves**. Do NOT
upload / `runReport` on the pod per tweak (uploads never overwrite → fresh path each time,
WAF-throttled, proves nothing while the layout is still moving). Pod upload + render happen **once,
after approval** — see rendering-and-running §7. `.xpt` is the exception (no local render — pod each
time).
